...
if your are on macOS you should add the `clang-11` and `compilers` packages to that list:

```
$ conda create --name sklworkshop -c conda-forge numpy scipy cython joblib threadpoolctl pytest matplotlib pandas clang-11 compilers
```
...
```
$ conda install -n sklworkshop clang-11 compilers  # there is a bug with clang-12
$ conda deactivate
$ conda activate sklworkshop
```
